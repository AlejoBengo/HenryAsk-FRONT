...
export const AnswerDetails = ({
  ...
  const handleCloseModalReport = () => {
    ...
    setInfoReportRes(infoReportRes={
      reason:'',
      description:'',
      owner:usuario,
      status:'PENDING',
  ...
  const handleChangeOption = (event:any) => {
    setInfoReportRes(infoReportRes = {...infoReportRes , reason:event.target.value});
  };
  ...
  const handleSubmitModalReport = () => {
    ...
    reportPost(infoReportRes).then((response)=> (setInfoReportRes(infoReportRes={
      reason:'',
      description:'',
      owner:usuario,
      status:'PENDING',
  ...
  const handleOpenModalReport = () => {
    ...
    setInfoReportRes(infoReportRes={...infoReportRes, answer:answerData})
  };
      ...
      <Grid container spacing={2}>
        ...
        <Grid item xs={2.7} sx={{display:"flex", justifyContent:"end", alignItems:"center"}}>
          ...
          <Menu
              ...
              >
                {user._id === answerData.owner._id && (<MenuItem onClick={handleOpenEdit}>Editar</MenuItem>)}
                {(user._id === answerData.owner._id || user.role>3) && (<MenuItem onClick={handleOpenDelete}>Eliminar respuesta *Rol admin*</MenuItem>)}
                {user._id !== answerData.owner._id && (<MenuItem onClick={handleOpenModalReport}>Reportar respuesta</MenuItem>)}
              </Menu>
            ...
            <DialogContent>
              ...
              <Grid container spacing={2} sx={{width:"40vw"}}>
                ...
                <Grid item xs={6} padding="0px" sx={{ display:"flex" , alignItems:"center" , height:"80px"}}>
                  <FormControl sx={{ minWidth: 120 ,  width:"100%"}} size="small" error={infoReportRes.reason?false : true}>
                ...
                <Grid item xs={6} sx={{display:"flex" , alignItems:"center"}}>
                  <Box display="flex" alignItems="center">{infoReportRes.reason}{infoReportRes.reason? <CheckIcon/> : null }</Box>
                </Grid>
                <Grid item xs={12}>
                  <StyledTextField
                    variant="filled"
        ...
        <Grid item xs={12} sx={{display:"flex", justifyContent:"start"}}>
          <Typography variant="body1" align="left" gutterBottom sx={{padding:"10px 10px 10px 25px"}}>
            {answerData.content}
          </Typography>
        </Grid>
      </Grid>
