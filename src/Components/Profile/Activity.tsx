...
export default function Activity() {
    ...
    maxSteps = reverseArr(userProfile.posts).length ? reverseArr(userProfile.posts).slice(0,6).length : 1;
    maxSteps1 = reverseArr(userProfile.answers).length ? reverseArr(userProfile.answers).slice(0,6).length : 1;
    maxSteps2 = reverseArr(userProfile.comments).length ? reverseArr(userProfile.comments).slice(0,6).length : 1;
                            ...
                            {reverseArr(userProfile.posts).slice(0, 6).map((step: any) => (
                                <Grid>
                                    <Grid sx={{ height: 230, display:'grid', padding:'1rem' }}>
                                        ...
                                        display='flex'
                                        justifyContent='center'>
                                            {step.question.length >= 100 ? `${content(step.question)}...` : step.question}
                                        </Typography>
                                        <Typography
                                        display='flex'
                                        justifyContent='center'
                                        variant="subtitle1">
                                            {step.description.length >= 100 ? `${content(step.description)}...` : step.description}
                                        </Typography>
                                    </Grid>
                            ...
                            {reverseArr(userProfile.answers).slice(0, 6).map((step: any) => (
                                <Grid>
                                    <Grid sx={{ height: 230, display:'grid', padding:'1rem' }}>
                                        ...
                                        justifyContent='center'
                                        variant="subtitle1">
                                            {step.content.length >= 100 ? `${content(step.content)}...`: step.content}
                                        </Typography>
                                    </Grid>
                                    <Grid display='flex' justifyContent='center' paddingBottom='0.5rem'>
                                        <Typography>
